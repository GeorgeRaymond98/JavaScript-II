...
runners.forEach(function(value){
  return fullNames.push(`Full name : ${value.first_name} ${value.last_name}`);
});
...
firstNamesAllCaps = runners.map(function(value){
...
runnersLargeSizeShirt = runners.filter(function(value){
...
ticketPriceTotal = runners.reduce(function(accumulator, value){ 
...
// Problem 1 Console.log All the rich runners
let newDonation = [];

newDonation = runners.filter(function(value){
...
emailCaps = runners.map(function(value){
  return value.email.toUpperCase();
});
console.log(emailCaps);
// Problem 3
let runnersCompany  = [];

runnersCompany = runners.map(function(value){
  return `${value.id} ${value.first_name} is running for ${value.company_name}` ;
});
console.log(runnersCompany);